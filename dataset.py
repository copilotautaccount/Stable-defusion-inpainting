...
class InpaintingDataset(Dataset):
    ...
    def _load_image(self, path: Path) -> Image.Image:
        return Image.open(path).convert("RGB").resize(self.image_size, Image.Resampling.LANCZOS)

    def _load_mask(self, path: Path) -> Image.Image:
        mask = Image.open(path).convert("L").resize(self.image_size, Image.Resampling.NEAREST)
        arr = np.array(mask)
